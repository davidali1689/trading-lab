"""Mid-cap sniper evaluator — $2B–$10B intraday gates."""
...
def evaluate_mid_cap_sniper(
    ctx: SessionContext,
    *,
    mode: RunMode = RunMode.BACKTEST,
    spec: MidCapSniperSpec = MID_CAP_SNIPER,
) -> SniperDecision:
    ...
    if ctx.market_cap_usd is not None:
        cap_ok = (
            ctx.market_cap_usd >= spec.min_market_cap_usd
            and ctx.market_cap_usd < spec.max_market_cap_usd
        )
